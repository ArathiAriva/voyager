...
- **Duplicate extraction cost.** Every message exchange re-sends the *entire* transcript (`conversations.py:88-92`) to the LLM and re-extracts from scratch, so a 20-message conversation would re-extract the same early preferences ~10 times. **Correction (measured 2026-08-22):** this analysis called it "a significant contributor to both the duplicate rate and the usage bill" without measuring it. It isn't, at current scale — conversations here are median 2 messages and max 4, so the redundancy is ~1% of turns sent, and memory extraction is only 2% of total LLM spend ($0.077 over 96 calls). The mechanism is real and scales quadratically, but the claim of present significance was wrong. Tracked as M-6 in OPEN-ITEMS.md with the condition for revisiting.
